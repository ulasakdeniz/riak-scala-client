...
class RiakSecondaryIndexesSpec extends RiakClientSpecification with RandomKeySupport {
  ...
  "A RiakBucket" should {
    "support storing and fetching a value with one Int index (by key and each index)" in new StoreAndFetch[ClassWithOneIntIndex](ClassWithOneIntIndex("bar"), "bar", ClassWithOneIntIndex.indexes) {}

    "support storing and fetching a value with one String index (by key and each index)" in new StoreAndFetch[ClassWithOneStringIndex](ClassWithOneStringIndex("bar"), "bar", ClassWithOneStringIndex.indexes) {}

    "support storing and fetching a value with two Int indexes (by key and each index)" in new StoreAndFetch[ClassWithTwoIntIndexes](ClassWithTwoIntIndexes("bar"), "bar", ClassWithTwoIntIndexes.indexes) {}

    "support storing and fetching a value with two Int indexes with the same index name (by key and each index)" in new StoreAndFetch[ClassWithTwoIntIndexesWithTheSameName](ClassWithTwoIntIndexesWithTheSameName("bar"), "bar", ClassWithTwoIntIndexesWithTheSameName.indexes) {}

    "support storing and fetching a value with multiple mixed indexes" in new StoreAndFetch[ClassWithMixedIndexes](ClassWithMixedIndexes("bar"), "bar", ClassWithMixedIndexes.indexes) {}
